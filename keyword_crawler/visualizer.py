...
class DataVisualizer:
    ...
    def generate_all_visualizations_async(self, days: int = 7, progress_callback: Optional[Callable] = None) -> Dict[str, str]:
        ...
        # 预先获取所有需要的数据（避免重复查询）
        update_progress(1, 7, "正在获取数据...")
        
        results_cache_key = f"results_{days}"
        tasks_cache_key = f"tasks_{days}"
        
        results_data = self._get_cached_data(
            results_cache_key, 
            self.db_manager.get_search_results_count, 
            days=days
        )
        
        task_data = self._get_cached_data(
            tasks_cache_key,
            self.db_manager.get_task_statistics,
            days=days
        )
        
        update_progress(2, 7, "数据获取完成，开始生成图表...")
        
        # 定义图表生成任务
        chart_tasks = [
            ('timeline', self._plot_timeline_with_data, results_data, days),
            ('keyword_comparison', self._plot_keyword_comparison_with_data, results_data, days),
            ('engine_performance', self._plot_engine_performance_with_data, task_data, days),
            ('activity_heatmap', self._plot_heatmap_with_data, task_data, days),
            ('interactive_dashboard', self._generate_dashboard_with_data, results_data, task_data, days)
        ]
        ...
        for future in as_completed(future_to_name):
            ...
            try:
                ...
                completed += 1
                update_progress(2 + completed, 7, f"已完成 {name} 图表")
            except Exception as e:
                print(f"生成 {name} 图表时出错: {e}")
                completed += 1
                update_progress(2 + completed, 7, f"{name} 图表生成失败")
        
        update_progress(7, 7, "所有图表生成完成!")
    ...
    def _plot_keyword_comparison_with_data(self, results_data: List[Dict], days: int) -> str:
        ...
        # 按关键词聚合数据
        keyword_stats = df.groupby('keyword')['result_count'].sum().reset_index()
        keyword_stats = keyword_stats.sort_values('result_count', ascending=True).tail(10)  # 取前10个
        
        if keyword_stats.empty:
            return None
        
        # 创建水平柱状图
        fig, ax = plt.subplots(figsize=(10, max(6, len(keyword_stats) * 0.5)))
        
        bars = ax.barh(keyword_stats['keyword'], keyword_stats['result_count'], 
                      color=sns.color_palette("viridis", len(keyword_stats)))
